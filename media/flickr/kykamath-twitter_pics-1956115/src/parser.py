...
class Utilities:
    ...
    @staticmethod
    def iterateTweetsFromGzip2(file):
        for line in gzip.open(file, 'rb'): 
            try:
                data = cjson.decode(line)
...
#                            print tweet, Settings.new_zealand_pics_folder+'tweets/'+Utilities.getDataFile(d)
    ...
    @staticmethod
    def downloadImagesUS():
        services = {'twitpic': HTMLParsers.parseTwitpic, 'yfrog': HTMLParsers.parseYfrog, 'twitrpix': HTMLParsers.parseTwitrpix}
        for f in ['2011_8_%s'%i for i in range(22, 30)]:
            for tweet in Utilities.iterateTweetsFromFile(Settings.US_pics_folder+'tweets/'+f):
            #for tweet in Utilities.iterateTweetsFromFile(Settings.new_zealand_pics_folder+'tweets/'+f):
                ...
                for service, parseMethod in services.iteritems():
                    if service in url:
                        id = tweet['id']
                        fileName = Settings.US_pics_folder+Utilities.getDataFile(d)+'/%s_%s.jpeg'%(str(d).replace(' ', '_'), id)
                        print url, fileName
    ...
    def downloadImages():
        services = {'twitpic': HTMLParsers.parseTwitpic, 'yfrog': HTMLParsers.parseYfrog, 'twitrpix': HTMLParsers.parseTwitrpix}
        for f in ['2011_3_%s'%i for i in range(13, 30)]:
...
#    Parser.downloadImages()
    ...
    Parser.getTweetsForUS()
    Parser.downloadImagesUS()
