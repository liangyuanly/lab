...
from graphOfTokens import transPoints2 #disOfTemp, disOfTemp2
...
def unfilteredOccTmpLoc(term_time, term_locs, term_occur, truth_file, cluster_num, format):
    global g_selected_words;

    dis_matrix_time = DisCalculator.disOfTemp(term_time)
    dis_matrix_locs = DisCalculator.disOfGeo(term_locs)

    dis_matrix = DisCalculator.disOfOccTimeGeo(term_occur, dis_matrix_time, dis_matrix_locs, g_selected_words);
...
def postIter(term_time, term_locs, term_occur, pre_theta_time_prob, pre_theta_loc_prob, pre_clusters, centers, cluster_num, truth_file, out_theta_file, dis_method, format, filter_type):
    global g_selected_words
    ...
    while isChanging:
        ...
        if testGoal(pre_theta_time_prob, theta_time_prob, pre_clusters, clusters) or testGoal(pre_theta_loc_prob, theta_loc_prob, pre_clusters, clusters) or iter >= 10:
            isChanging = 0;
            if iter == 1:
                ret_puri = puri;
                ret_accu = accu;
            ...
            pre_theta_time_prob = theta_time_prob;
            pre_theta_loc_prob = theta_loc_prob;
...
def iterThetaWordTime(event, cluster_num, format, dis_method, filter_type, map):
    #first use the co-occur as the metric to cluster tokens
    prob_theta_time = {};
    prob_word_theta = {};
    metrics = {};
    ...
    pre_clusters, centers, term_occur = initClusters(event, cluster_num, None, format, 'coocur', dis_method); 
    theta_time_prob = getThetaTimeProb2(pre_clusters, term_prob, cluster_num); 
    
    #theta_time_prob, theta_prob = getWeightThetaTimeProb(pre_clusters, centers, term_prob, cluster_num); 
    
    #term_sim_occ = DisCalculator.disOfOccur(term_occur);
    ...
    term_locs = transPoints2(term_locs, 0, map, 0, 100); 
    theta_loc_prob = getThetaTimeProb2(pre_clusters, term_locs, cluster_num); 
    
    ret, accu = iterFunc(term_prob, term_locs, term_occur, theta_time_prob, theta_loc_prob, pre_clusters, centers, cluster_num, truth_file, out_theta_file, dis_method, format)
    ...
    metrics['unfiltered'], accu_metrics['unfiltered'] = unfilteredOccTmpLoc(term_prob, term_locs, term_occur, truth_file, cluster_num, format)
    
    filename = 'data/event/' + event + '/term_coocurence2.txt'
    infile = file(filename);
    if format == 'utf-8':
        dis_matrix = json.load(infile, format);
    else:
...
def iterFunc(term_prob, term_locs, term_occur, theta_time_prob, theta_loc_prob, pre_clusters, centers, cluster_num, truth_file, out_theta_file, dis_method, format):
    metrics = {};
    accu_metrics = {};
    #post probability iteration
    theta_time_prob_temp = deepcopy(theta_time_prob);
    theta_loc_prob_temp = deepcopy(theta_loc_prob);
    cluster_temp = deepcopy(pre_clusters);
    centers_temp = deepcopy(centers);
    print 'post iter begin.......'
    puri, accu, cluster_post = postIter(term_prob, term_locs, term_occur, theta_time_prob_temp, theta_loc_prob_temp, cluster_temp, centers_temp, cluster_num, truth_file, out_theta_file, dis_method, format, [])
    metrics[1] = puri;
    accu_metrics[1] = accu;
...
def termIterEventMain(event, cluster_num, format, filter_type, map):
    global g_selected_words
    
    outfilename = 'data/event/' + event + '/results_occ_tem_loc.txt';
    outfile = file(outfilename, 'w');
    ...
    for i in range(0, iter_count):
        ...
        metric, accu_metric = iterThetaWordTime(event, cluster_num, format, dis_method, filter_type, map);
...
def eventMain():
    ...
    cluster_num = 5;
    bb = getJPbb();
    termIterEventMain(event, cluster_num, format, filter_type, bb)
