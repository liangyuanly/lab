...
def get_checkin(filename):
    ...
    infile = open('data/checkin_data.txt');
...
#    stored_array = p.load(outfile);
    ...
    #load the tweet type
    infile = file('data/ordered_tweet_type.txt');
...
def duration_stat2(venue_key):
    ...
    checkin_24 = [0 for i in range(0, 24)];
    ...
    for i in checkin_list:
        ...
        userlist.append(i);
        h = hour(item[4]);
        checkin_24[h-1] = checkin_24[h-1] + 1;
    ...
    outfile = file('output/top_venue_duration_m2.txt', 'a');
...
def duration_stat(venue_key):
    ...
    for key in user_map.keys():
        ...
        for j in range(0, len(list)):
            ...
            #this method the duration is calculated with the checkin in L and next checkin not in L
...
def duration_of_top_venue():
    outfile = file('output/top_venue_duration_m2.txt', 'w');
    outfile.close();
    outfile = file('output/top_venue_period.txt', 'w');
    outfile.close();
    outfile = file('output/top_venue_weight.txt', 'w');
    outfile.close();
    
    # load the arrays and maps
    get_checkin('./data/stat.txt');

    infile = file('./data/venue_name_ids.txt', 'r');
    checkin_outfile= file('output/venue_checkin_24h.txt', 'w');
    while 1:
        checkin_24 = [0 for i in range(0, 24)];
        name = infile.readline();
        if not name:
        ...
        outfile = file('output/top_venue_weight.txt', 'a');
        outfile.write(name);
        outfile.close();


        outfile = file('output/top_venue_period.txt', 'a');
        outfile.write(name);
        outfile.close();
        ...
        for id in ids:
            ...
            id = id + '\n';
            print id;
            #use method 2 to calculate the duration
            checkin_24_tmp = duration_stat2(id);
            checkin_24 = checkin_24 + checkin_24_tmp;
        checkin_outfile.write(name);
        for i in range(0, 24):
            checkin_outfile.write(str(checkin_24[i])+'\t');
        checkin_outfile.write('\n');
